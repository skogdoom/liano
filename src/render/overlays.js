...
// Title, game over and pause screens, and the off-screen indicator.
export class Overlays {
  constructor() {
    ...
    place(this.title, text('SPACE  ·  let go', 22, { color: GOLD, weight: 'bold' }), 70);
    this.titlePrompt = place(this.title, text('Press Space to start', 28), 125);
    ...
    this.gameOverPrompt = place(this.gameOver, text('Press Space to play again', 28), 125);

    this.paused = panel(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 420, 170);
    place(this.paused, text('Paused', 64, { weight: 'bold' }), -25);
    place(this.paused, text('Click the game to resume', 22), 45);
